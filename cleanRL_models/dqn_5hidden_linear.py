# DQN with 5 hidden layers, all linear (no ReLU). Based on dqn_4layers_linear.py
import os
import random
import time
...
# 5 hidden layers (120, 84, 84, 84, 84), all linear — no ReLU or other activations
...
if __name__ == "__main__":
    ...
    # Track episode return and length per env so we always log to TensorBoard when an episode ends
    ...
    for global_step in range(args.total_timesteps):
        ...
        # TRY NOT TO MODIFY: record rewards for plotting purposes (prefer final_info when present)
        if "final_info" in infos:
            for idx, info in enumerate(infos["final_info"]):
                if info and "episode" in info:
                    ...
                    ep_returns[idx] = 0
                    ep_lengths[idx] = 0
        # Fallback: log when episode ends (termination or truncation) using tracked return/length
