...
class LearnableGateParams(nn.Module):
    """
    Scalar nn.Parameters for confidence-gating and controller equations.
    All initialized to replicate the original hard-coded values so the
    learnable model starts at the same operating point.

      1–2  : gate_k_min, gate_k_max  (affine gate range)
    ...
    def __init__(self):
        ...
        # 3: softmax temperature (log-space; softplus(0) ≈ 0.69 initially)
        self.log_temp    = nn.Parameter(torch.tensor(0.0))
        # 4, 12: trunk scale = softplus(log_trunk_scale) * (1 + slope * m)
        self.log_trunk_scale = nn.Parameter(torch.tensor(0.5413))  # softplus ≈ 1.0
    ...
    def get_trunk_scale(self, m: torch.Tensor) -> torch.Tensor:
        base = F.softplus(self.log_trunk_scale)
        return base * (1.0 + self.trunk_m_slope * m.detach())
    ...
    def snapshot(self) -> Dict:
        ...
        return {
            "gate_k_min":      float(self.gate_k_min.item()),
            "gate_k_max":      float(self.gate_k_max.item()),
            "temp":            float(self.get_temp().item()),
...
class BFeedbackConfGateNetwork(nn.Module):
    ...
    def _gate_k(self, Q_cart: torch.Tensor, Q_pole: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        When self.learnable_gate is True:
          - Z-norm eps is hard-coded (Z_NORM_EPS); softmax temperature (item 3) from gate_params
          - routing C stays in-graph (no .detach())
          - control signal m is computed as a tensor (item 5)
          - dynamic k_min (items 9-11) replaces self.gate_k_min
          - 'm_tensor' key is added to stats for use in forward() trunk scaling
        """
        ...
        if self.learnable_gate:
            ...
            # Softmax with learnable temperature (item 3); in-graph so grad flows
            temp = gp.get_temp()
            C = F.softmax(torch.stack([m_c_norm, m_p_norm], dim=1) / temp, dim=1)
            # Control signal m (item 5)
            raw = (m_c_norm + m_p_norm).abs().mean()
            m_tensor = raw / gp.get_m_divisor()
            # Dynamic k_min (items 9-11)
            k_min_eff = torch.min(
                gp.ctrl_kmin_cap,
                gp.ctrl_kmin_base + gp.ctrl_kmin_slope * m_tensor,
            )
            km, kx = k_min_eff, gp.gate_k_max
        else:
            eps = 1e-8
            m_c_norm = (m_c_raw - mc_mean) / (mc_std + eps)
            m_p_norm = (m_p_raw - mp_mean) / (mp_std + eps)
        ...
        if m_tensor is not None:
            # Keep tensor in stats for forward() trunk scaling; float for logging
    ...
    def forward(
        ...
        if self.learnable_gate and self._gating_active() and m_stats is not None and "m_tensor" in m_stats:
            # Items 4 + 12: trunk_scale = softplus(log_trunk_scale) * (1 + slope * m)
            z_trunk = z_trunk * self.gate_params.get_trunk_scale(m_stats["m_tensor"])
        elif self.trunk_scale != 1.0:
...
def run_one(
    ...
    while t < total_timesteps and (
        ...
        if t > learning_starts and t % train_frequency == 0:
            ...
            # Q_cart/Q_pole (via in-graph C), and opt_aux needs that graph too
            effective_loss.backward(retain_graph=bool(learnable_ok))
            if main_grad_clip > 0:
                params_to_clip = (
                    list(q_net.linear_feature.parameters())
                    + list(q_net.trunk.parameters())
            ...
            if learnable_ok and t % LOG_EVERY == 0:
                snap = q_net.gate_params.snapshot()
...
def _ckpt_path_learnable(ckpt_dir: Path, seed: int) -> Path:
...
def _load_result_learnable(ckpt_dir: Path, seed: int) -> Dict:
    path = _ckpt_path_learnable(ckpt_dir, seed)
    ckpt = torch.load(path, map_location="cpu", weights_only=False)
    ep      = ckpt["episode_returns"]
    last100 = float(np.mean(ep[-100:])) if len(ep) >= 100 else float(np.mean(ep)) if ep else 0.0
    return {
...
def plot_learnable_vs_baseline(
    ...
    """Baseline (no gate) vs learnable-gate comparison."""
    COLOR_LEARN = "#d62728"
    ...
    ax.plot(ep_l, sm_l, color=COLOR_LEARN, lw=2.2, label=f"learnable gate       last-100: {l100_l:.1f}")
    ax.axhline(500, color="gray", lw=1.0, ls="--", alpha=0.6, label="max (500)")
    ax.set_xlabel("Episode", fontsize=12)
    ax.set_ylabel("Episodic return", fontsize=12)
    ax.set_title(
        f"Conf. gate (learnable params): baseline vs learnable  "
...
def plot_gate_params_history(
    ...
    """Plot how each LearnableGateParams value evolves over training."""
    ...
    for ax, key in zip(axes, keys):
        ...
        if rows.shape[0] > 1:
            ...
            ax.plot(grid, rows[0], color="#1f77b4", lw=2.0)
...
def main() -> None:
    ...
    if args.learnable:
        ...
        # Reuse baseline from the step-cap run when doing an episode-targeted sweep
        baseline_src_dir = (
            ROOT / "paper" / "_tmp_b_feedb_cg" / f"ckpt_learnable_{decay_tag}"
            if n_ep_target is not None else learn_ckpt_dir
        )

        if not args.plot_only:
            # ---- baseline (reuse step-cap run only if it already reached n_ep_target) ----
            reuse_baseline = (
            ...
            if reuse_baseline:
                ...
                if base_seeds_to_run:
                    ...
                    t0 = time.perf_counter()
                    for seed in base_seeds_to_run:
                        run_one(seed, n_step, False, device,
            ...
            # ---- learnable gate ----
            learn_seeds_to_run = [
                s for s in seeds if not _ckpt_path_learnable(learn_ckpt_dir, s).exists()
            ] if args.train_missing_only else list(seeds)
            if learn_seeds_to_run:
                ep_str = f"up to {n_ep_target:,} ep" if n_ep_target else f"{n_step:,} steps"
                print(f"\n>>> [LEARNABLE] Training seeds {learn_seeds_to_run} for {ep_str} ...", flush=True)
                t0 = time.perf_counter()
        ...
        base_load_dir = baseline_src_dir if (
            n_ep_target is not None
            and baseline_src_dir != learn_ckpt_dir
            and all(_ckpt_path(baseline_src_dir, s, False).exists() for s in seeds)
        ) else learn_ckpt_dir
        ...
        for seed in seeds:
            ...
            if not _ckpt_path_learnable(learn_ckpt_dir, seed).exists():
                raise FileNotFoundError(
                    f"Missing learnable checkpoint for seed {seed} in {learn_ckpt_dir}"
                )

        base_res  = [_load_result(base_load_dir, s, False) for s in seeds]
        learn_res = [_load_result_learnable(learn_ckpt_dir, s) for s in seeds]

        print("\n=== Final-100-episode summary (learnable gate) ===", flush=True)
